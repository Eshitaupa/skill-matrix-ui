...
// const API_BASE =
//   process.env.REACT_APP_API_BASE || "https://skill-matrix-api-uat-c8ajcqhhaxevbvam.northcentralus-01.azurewebsites.net";
// const TENANT_ID =
...
//   // CHANGED: tracks the "logged in but Databricks lookup failed" state,
//   // separately from a real "can't reach the backend at all" failure.
//   const [disciplineWarning, setDisciplineWarning] = useState(false);
...
//       // CHANGED: surface the soft warning from the backend instead of
...
//       {/* CHANGED: separate, softer banner for a discipline-lookup-only failure.
//           The user is still authenticated and can use the app; this just
//           tells them their discipline access couldn't be confirmed. */}
//       {!checkError && disciplineWarning && (
//         <div style={warningBannerStyle}>
//           ⚠ Could not confirm your discipline access right now. You're signed in,
//           but some disciplines may not be visible. Try refreshing shortly.
//         </div>
//       )}
...
// // CHANGED: new style for the soft discipline-lookup warning, visually
// // distinct (blue, not amber) from the hard "server unreachable" banner.
...
const API_BASE =
  process.env.REACT_APP_API_BASE || "https://skillmatrixapi-cjhgezceakbxe3ej.northcentralus-01.azurewebsites.net";
const TENANT_ID =
  process.env.REACT_APP_TENANT_ID || "bfbb9a2b-6d99-4e78-b3c7-95005d555c8b";
const CLIENT_ID =
  process.env.REACT_APP_CLIENT_ID || "f40b4118-24a7-4c0e-a5e9-1b3cad660d2e";
  ...
  useEffect(() => {
    ...
    async function completeLogin() {
      try {
        ...
        // NOTE: /api/auth/session no longer returns allowedDisciplines
        // (that Databricks lookup was redundant — /api/auth/me does it
        // right after we land on /home). Just stash the email here;
        // checkSession() will populate allowedDisciplines properly.
        sessionStorage.setItem("userEmail", sessionData.email || "");
...
function App() {
  ...
  // Tracks the "logged in but Databricks lookup failed" state,
  // separately from a real "can't reach the backend at all" failure.
  const [disciplineWarning, setDisciplineWarning] = useState(false);
  ...
  return (
    <ExportProvider>
      ...
      {/* Separate, softer banner for a discipline-lookup-only failure.
          The user is still authenticated and can use the app; this just
          tells them their discipline access couldn't be confirmed. */}
      {!checkError && disciplineWarning && (
        <div style={warningBannerStyle}>

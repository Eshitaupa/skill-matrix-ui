...
function transformApiToMatrix(rows, roleLevels) {
  ...
  (rows || []).forEach((row) => {
    ...
    const sortOrder = Number(row.SortOrder ?? row.sort_order ?? 999999);
    ...
    let skillObject = groups[categoryKey].skills.find(
    ...
    if (!skillObject) {
      ...
      groups[categoryKey].skills.push(skillObject);
    }

    if (sortOrder < Number(skillObject.sortOrder ?? 999999)) {
...
function filterOutDeletedRows(base, deletedKeySet) {
  if (!deletedKeySet || deletedKeySet.size === 0) {
    return base;
  }
  ...
  return (base || []).map((group) => ({
    ...
    skills: (group.skills || []).map((skill) => {
      ...
      Object.entries(edits || {}).forEach(([key, value]) => {
        const [category, subskill, level] = key.split("|");

        if (
          keyOfText(category) === keyOfText(group.category) &&
          keyOfText(subskill) === keyOfText(skill.name)
        ) {
          levels[level] = value;
          changed = true;
        }
...
export default function SkillMatrix({ allowedDisciplines = [] }) {
  /* ---------------------------------------------------------
     STATE
  --------------------------------------------------------- */
  ...
  /*
   * Split the old single "actionBusy" flag into per-action flags.
   * Previously ALL async actions (save / delete / add) shared one
   * flag, which meant that adding a row kept the "Edit" and
   * "Add Row" toolbar buttons disabled/uneditable until the network
   * request for that add finished. Now each action only disables
   * the controls that are actually relevant to it.
   */
  ...
  const fetchMatrixRef = useRef(null);
  ...
  useEffect(() => {
    ...
    async function loadMeta() {
      try {
        const response = await fetch(`${API_SKILL}/meta?t=${Date.now()}`, {
          method: "GET",
          credentials: "include",
          cache: "no-store",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            data?.message || `Meta request failed with status ${response.status}`
          );
        }
        ...
        const backendAllowed = Array.isArray(data?.allowedDisciplines)
        ...
        setMeta({
          disciplines,
          roles,
          allowedDisciplines: backendAllowed,
        });
  ...
  const disciplineOptions = useMemo(() => {
    const hasAll = effectiveAllowedDisciplines.some((item) => {
  ...
  }, [effectiveAllowedDisciplines, meta.disciplines]);
  ...
  useEffect(() => {
    ...
    if (!onlyDiscipline || ["all", "all disciplines"].includes(keyOfText(onlyDiscipline))) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const fixedRole = DISCIPLINE_ROLE_MAP[filters.discipline] || "";
  ...
  const fetchMatrix = useCallback(
    async ({ silent = false, forceServerTruth = false } = {}) => {
      ...
      const firstLoad = !matrixLoadedRef.current;
      ...
      try {
        const url =
          `${API_SKILL}` +
          `?discipline=${encodeURIComponent(filters.discipline)}` +
        ...
        if (!response.ok) {
          ...
          throw new Error(message || `Matrix request failed with ${response.status}`);
        }
        ...
        if (forceServerTruth) {
          ...
          setMatrixData(filterOutDeletedRows(transformed, deletedRowKeysRef.current));
        }
        ...
        showToast("Could not load the matrix.", "error");
      } finally {
    ...
    [filters.discipline, filters.role, showToast]
  );
  ...
  useEffect(() => {
    ...
    fetchMatrix();
  }, [fetchMatrix, filters.discipline, filters.role]);
  ...
  useEffect(() => {
    ...
    source.onmessage = (event) => {
      try {
        if (savingBusy || deletingBusy || addingBusy || isEditMode) {
          return;
        }
  ...
  }, [filters.discipline, filters.role, savingBusy, deletingBusy, addingBusy, isEditMode]);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      showToast("Latest changes loaded.", "success");
    } finally {
  ...
  function toggleSelectedRow(category, subskillName) {
    ...
    setSelectedRows((previous) => {
      const exists = previous.some(
        (item) => String(item).toLowerCase() === key.toLowerCase()
      );
  ...
  function toggleCategoryRows(category, subskillNames) {
    ...
    setSelectedRows((previous) => {
      const previousLower = new Set(
        previous.map((item) => String(item).toLowerCase())
      );

      const allSelected =
        keys.length > 0 &&
        keys.every((key) => previousLower.has(key.toLowerCase()));

      if (allSelected) {
        const removeSet = new Set(keys.map((key) => key.toLowerCase()));

        return previous.filter(
          (item) => !removeSet.has(String(item).toLowerCase())
        );
      }
      ...
      keys.forEach((key) => {
        if (!previousLower.has(key.toLowerCase())) {
          result.push(key);
        }
      });
  ...
  function requestDeleteSelectedRows() {
    if (!selectedRows.length) {
      showToast("Select at least one subskill.", "error");
      ...
      .map((key) => {
        ...
        for (const group of matrixData) {
          for (const skill of group.skills || []) {
            if (rowKeyLower(group.category, skill.name) === String(key).toLowerCase()) {
              return {
  ...
  async function confirmDeleteRows() {
    if (deletingBusy || !confirmDelete?.rows?.length) {
      return;
    }
  ...
  /*
   * When the selected discipline is one of the fixed-role
   * disciplines (Project Management, Piping Engineering,
   * Mechanical) the Role field must be frozen to "Engineer",
   * matching the behavior of the main filter bar.
   */
  const isAddRoleLocked = Boolean(DISCIPLINE_ROLE_MAP[form.discipline]);

  const modalSkillOptions = useMemo(() => {
    return (matrixData || []).map((group) => group.category).filter(Boolean);
  }, [matrixData]);
  ...
  const modalSubskillOptions = useMemo(() => {
    ...
    const group = (matrixData || []).find(
      (item) => keyOfText(item.category) === keyOfText(form.skill)
    );

    return (group?.skills || []).map((skill) => skill.name).filter(Boolean);
  }, [matrixData, form.skill]);
  ...
  async function handleAddRow() {
    ...
    if (!discipline || !role || !category || !subskill) {
      showToast("Fill Discipline, Role, Skill Category and Subskill.", "error");

      return;
    }

    const existingCategory = (matrixData || []).find(
    ...
    if (duplicate) {
      showToast("This Skill and Subskill already exists.", "error");
    ...
    const optimisticSkill = {
      ...
      levels: Object.fromEntries(levels.map((level) => [level, "NA"])),
    };
     ...
     */
    deletedRowKeysRef.current.delete(rowKeyLower(category, subskill));
    ...
    setMatrixData((previous) => {
      ...
      if (exists) {
        return previous.map((group) => {
          if (keyOfText(group.category) !== keyOfText(category)) {
            return group;
          }
          ...
          return {
            ...
            skills: [...(group.skills || []), optimisticSkill],
          };
        });
      }
      ...
      return [
...

        {
          ...
          skills: [optimisticSkill],
        },
      ];
    });

    /*
     * Close the modal and clear the add-row busy state right away
     * so the toolbar (Edit / Add Row / Refresh) stays fully usable
     * while this save quietly completes in the background. Only
     * the Add Row modal itself is gated by addingBusy, and that
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || "Add failed");
      }

      showToast("Skill added and saved.", "success");
  ...
  function exportToExcel() {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([...headerRows, tableHeader, ...tableData]);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Project Meridian");
  ...
  function exportToPDF() {
    ...
    document.text("Project Meridian", 14, 12);

    document.setFontSize(10);

    document.text(`Discipline: ${filters.discipline || "-"}`, 14, 18);

    document.text(`Role: ${filters.role || "-"}`, 14, 23);

    document.text(`Level: ${filters.level || "All levels"}`, 14, 28);
    ...
    document.save(`Skill_Matrix_${filters.discipline}_${filters.role}${levelPart}.pdf`);
  }
    ...
    <div className="page-container">
      <style>{`
        ...
        .smf-loading-spinner {
          ...
          animation: smfSpin 0.7s linear infinite;
        }
        ...
        .sm-add-modal select:disabled,
        .sm-add-modal input:disabled {
          background: #f1f5f9;
          color: #64748b;
          cursor: not-allowed;
        }
      ...
      <div className="sticky-filters">
        <Filters
          ...
          canExport={filteredMatrixData.length > 0}
      ...
      <div className="smf-toolbar-row">
        <div className="smf-toolbar-left">
          <button
            ...
            disabled={savingBusy || deletingBusy || !filters.discipline || !filters.role}
            type="button"
          >
            ✏ Edit
          </button>
        ...
        {isEditMode && (
          <div className="smf-toolbar-right">
            {selectedCount > 0 && (
              <span className="smf-selected-pill">{selectedCount} selected</span>
            )}

            <button
              className="smf-clear-btn"
              onClick={clearSelectedRows}
              disabled={deletingBusy || selectedCount === 0}
              type="button"
            >
              Clear
            </button>
      ...
      <div className="table-hover-wrapper">
        <div className="table-responsive">
          {initialLoading && matrixData.length === 0 ? (
          ...
          <div className="sm-add-modal">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const role = DISCIPLINE_ROLE_MAP[discipline] || "";
            ...
            <select
              ...
              disabled={addingBusy || isAddRoleLocked}
            >
            ...
            {isAddRoleLocked && (
              <p className="sm-add-hint">
                Role is locked to Engineer for this discipline.
              </p>
            )}
            ...
            {form.isNewSkill ? (
              ...
              <select
                value={form.subskill}
                disabled={!form.skill || addingBusy}
                onChange={(event) => {
                  const value = event.target.value;
        ...
        <div className="smf-modal-overlay">
          <div className="smf-delete-card">
            <div className="smf-delete-card-header">
              ...
              <div>
                ...
                <p className="smf-delete-subtitle">
                  Selected subskills will be deleted and saved immediately.
                </p>
              </div>
            </div>
            ...
            <div className="smf-delete-card-body">
              <div>
                Delete <strong>{confirmDelete.rows.length}</strong> subskill
                {confirmDelete.rows.length > 1 ? "s" : ""}?
              </div>
              ...
              <div className="smf-delete-list">
                {confirmDelete.rows.map((row) => (
                  <div
                    ...
                    key={rowKey(row.category, row.subskillName)}
                  >
                    <div className="smf-delete-category">{row.category}</div>

                    <div className="smf-delete-subskill">{row.subskillName}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="smf-delete-card-footer">
              ...
              <button
                ...
                disabled={deletingBusy}
              >
                {deletingBusy ? "Deleting..." : "Delete & Save"}
              </button>
            </div>
          </div>
        </div>
      ...
      {toast && <div className={`smf-toast ${toast.tone}`}>{toast.message}</div>}
    </div>
  );
}

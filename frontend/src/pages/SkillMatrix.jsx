...
// ✅ Fallback used only if /meta fails, times out, or returns an empty list.
// Keeps "All"-access users from ever seeing a blank discipline dropdown.
...
// ✅ allowedDisciplines comes from App.js (e.g. ["Electrical"] or ["All"])
export default function SkillMatrix({ allowedDisciplines = [], userEmail = "" }) {
  const [filters, setFilters] = useState({ discipline: "", role: "", level: "" });
  const [refreshKey, setRefreshKey] = useState(0);
  const [matrixData, setMatrixData] = useState([]);
  ...
  // ✅ Load meta from API — always falls back to DEFAULT_DISCIPLINES rather
  // than leaving "All"-access users with an empty dropdown.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!res.ok) {
          console.error("META API FAILED:", res.status);
          if (!cancelled) {
            setMeta({ disciplines: DEFAULT_DISCIPLINES, roles: ["Engineer", "Designer"] });
            setMetaError(true);
          }
          return;
        }
        ...
        const disciplines = Array.isArray(data.disciplines) && data.disciplines.length
          ? data.disciplines
          : DEFAULT_DISCIPLINES;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${API_SKILL}?discipline=${encodeURIComponent(d)}&role=${encodeURIComponent(r)}`,
          { credentials: "include" }
        );
        if (!res.ok) { if (alive) setModalMatrix([]); return; }
        const data = await safeJson(res);
  ...
  function exportToPDF() {
    ...
    autoTable(doc, {
      ...
      body: [["NA", "Not Applicable"], ["1", "Familiar"], ["2", "Working Level"], ["3", "Extensive"], ["4", "Authoritative"]],
      styles: { fontSize: 9 },
  ...
  return (
    <div className="page-container">
      {metaError && (
        ...
        >
          ⚠ Couldn't reach the discipline list from the server — showing a default list instead.
        </div>
      )}

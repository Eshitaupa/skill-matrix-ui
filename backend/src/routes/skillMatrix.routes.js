...
const ACCESS_TABLE = "ogc_techdept_test.skill_matrix.user_discipline_access";
...
function getUserEmail(req) {
  return String(req.cookies?.user_email || "").trim().toLowerCase();
}

function levelsForRole(role) {
  return keyOf(role) === "designer" ? DESIGNER_LEVELS : ENGINEER_LEVELS;
}
...
async function requireDisciplineAccess(req, res, requestedDiscipline) {
  ...
  if (!email) {
    res.status(401).json({
      message: "User session email is missing. Please log out and log in again.",
    });
    return null;
  }
...
function setCachedMatrix(discipline, role, data) {
  matrixCache.set(cacheKey(discipline, role), {
    data,
    expiresAt: Date.now() + CACHE_TTL_MS,
  });
}

function invalidateMatrixCache(discipline, role) {
...
const SKILL_ORDER_CASE = `
    CASE
        ...
        WHEN LOWER(Discipline) = 'piping engineering' AND LOWER(Skill) = 'materials' THEN 4
        ...
        WHEN LOWER(Discipline) = 'instrumentation' AND LOWER(Skill) = 'valves' THEN 4
        WHEN LOWER(Discipline) = 'instrumentation' AND LOWER(Skill) = 'control & automation' THEN 5
        WHEN LOWER(Discipline) = 'instrumentation' AND LOWER(Skill) = 'modelling' THEN 6
        ...
        WHEN LOWER(Discipline) = 'project management' AND LOWER(Skill) = 'project planning & control' THEN 4
        WHEN LOWER(Discipline) = 'project management' AND LOWER(Skill) = 'commercial & cost' THEN 5
        WHEN LOWER(Discipline) = 'project management' AND LOWER(Skill) = 'risk & construction' THEN 6
        WHEN LOWER(Discipline) = 'project management' AND LOWER(Skill) = 'business & knowledge' THEN 7
...
router.get("/meta", async (req, res) => {
  try {
    ...
    const visibleDisciplines = hasAllAccess(allowedDisciplines)
      ? DISCIPLINES
      : DISCIPLINES.filter((discipline) =>
          allowedDisciplines.some((allowed) => keyOf(allowed) === keyOf(discipline))
        );
  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        ${normDispSql("Role")} AS Role,
        ${normDispSql("LevelKey")} AS LevelKey,
        ${normDispSql("Skill")} AS Skill,
        ${normDispSql("Subskill")} AS Subskill,
        Value,
        COALESCE(SortOrder, 999999) AS SortOrder
      FROM ${UPLOAD}
      WHERE 1 = 1
        ${disciplineFilter}
        ${roleFilter}
...
router.post("/save", async (req, res) => {
  try {
    ...
    for (const row of cleanedRows) {
      if (!row.Discipline || !row.Role || !row.LevelKey || !row.Skill || !row.Subskill) {
    ...
    for (const row of cleanedRows) {
      if (!isDisciplineAllowed(row.Discipline, access.allowedDisciplines)) {
        return res.status(403).json({
          message: `You do not have access to ${row.Discipline}.`,
          allowedDisciplines: access.allowedDisciplines,
        });
